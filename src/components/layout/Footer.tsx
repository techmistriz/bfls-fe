...
export default function Footer() {
  return (
    <footer className="relative w-full bg-[#333333] font-sans text-[13px] text-[#eeeeee]">
      {/* ================= MAIN FOOTER ================= */}
      <div className="mx-auto grid min-h-[523px] w-[1125px] max-w-[calc(100%-40px)] grid-cols-[300px_500px_250px] gap-[19px] pt-[47px] font-roboto max-lg:w-[calc(100%-40px)] max-lg:grid-cols-1 max-lg:gap-[40px] max-lg:pt-[40px] max-sm:w-[calc(100%-30px)] max-sm:max-w-none max-sm:gap-[35px] max-sm:pt-[35px]">
        {/* ================= CONTACT ================= */}
        <div className="w-full min-w-0">
          ...
          {contactPeople.map((person) => (
            <div key={person.email} className="mb-[23px] text-[15px]">
              <strong className="mb-[6px] block font-bold text-[#f2f2f2]">
                {person.name}
              </strong>

              <span className="block leading-[1.55]">{person.role}</span>

              <a
                href={`mailto:${person.email}`}
                className="block break-all font-semibold leading-[1.55] text-white underline hover:text-[#f58220]"
              >
                {person.email}
              </a>

              <span className="block leading-[1.55]">{person.phone}</span>
            </div>
          ))}
        ...
        <div className="w-full min-w-0">
          ...
          {summitShowcase.map((summit, i) => (
            ...
            >
              ...
              <span className="block leading-[1.5]">{summit.description}</span>

              <span className="block leading-[1.5]">{summit.location}</span>
      ...
      {/* ================= BOTTOM BAR ================= */}
      <div className="mx-auto mt-[50px] flex min-h-[87px] w-[1170px] max-w-[calc(100%-40px)] items-center justify-center border-t border-[#777777] max-lg:h-auto max-lg:min-h-0 max-lg:flex-col max-lg:items-start max-lg:gap-[15px] max-lg:px-[20px] max-lg:py-[25px] max-sm:w-[calc(100%-30px)] max-sm:max-w-none max-sm:px-0">
        <div className="flex w-[390px] flex-col leading-[1.55] max-lg:w-full">
          ...
          <span className="text-[13px] text-[#dddddd]">
            ...
            <a href="#" className="text-[#777777] hover:text-[#f58220]">
              pexels.com
            </a>
          </span>
        </div>
        ...
        <div className="w-[200px] whitespace-nowrap text-[13px] font-bold text-[#eeeeee] max-lg:w-full">
      ...
      <a
        href="#"
        aria-label="WhatsApp"
        className="fixed bottom-[78px] right-[17px] z-[99] flex h-[41px] w-[41px] items-center justify-center rounded-[10px] bg-[#25D366] text-white shadow-[0_1px_5px_rgba(0,0,0,0.35)] transition-transform hover:scale-105 max-sm:bottom-[65px] max-sm:right-[12px] max-sm:h-[40px] max-sm:w-[40px]"
      >
        <svg
          viewBox="0 0 24 24"
      ...
      {/* ================= RECAPTCHA ================= */}
      <div className="fixed bottom-[10px] right-0 z-[98] flex h-[50px] w-[54px] items-center justify-center rounded-l-[3px] bg-[#fafafa] shadow-[0_1px_5px_rgba(0,0,0,0.3)]">
        <div className="text-[30px] font-bold text-[#4285f4]">↻</div>
      </div>
    </footer>
  );
}
